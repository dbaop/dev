...
class Test_Page():
    ...
    def match(self,credit_no):
        ...
        rs_m = Test_Page().Mongodb_link('T4', 'ccbscf-biz-payment', 'billInfoDO',
            {"creditDetails.fkCredit": credit_no},
            {"_id": 1, "loanTransferDepositBill": 1, "creditDetails.feeAmount": 1,"corpName":1,"finFeeAmount":1,"platFeeAmount":1})
        print(rs_m)
        print(rs_m[0]['_id'])
            ...
            if rs[0][2] == 'PAYROLL_CREDIT':
                ...
                for j in range(0, len(c_depo)):
                    if isDeposit == 'True':
                        ...
                        data1 = {"_id":credit_no}
                        ts = Test_Page().Mongodb_count('T4', 'ccbscf-biz-payment', 'depositFeeFeedbackInfoDO', data1)
                        print(ts)
                        print(ts[0])
                            ...
                            post_data = \
                                {
                                    ...
                                    "feeReceiverAccountCode": bamf[0][0],
                                    "receivableMgtFee": rs_m[j]['creditDetails'][0]['platFeeAmount'],
                                    "processState": "WAIT",
