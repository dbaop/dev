...
class test_redeem_core(unittest.TestCase):
    ...
    def test_corp(self):
        ...
        count_order = Test_Page().Oracle_link('T4',
             "select count(*) from prod_ccbscf.t_product_corp t where t.corp_name in ('中通钢构')and \
        ...
        rs = Test_Page().Oracle_link('T4',
           "select t.user_type from prod_ccbscf.t_ci_user t where t.pk_user = (select c.pk_corp  from\
            prod_ccbscf.T_ci_corp c where c.corp_name = '中通钢构' )", 0, 2)
        assert rs[0][0] == 'CORE'
        print("该企业开通了订单融资，产品开通表中有%s条记录，企业类型为%s，显示单笔赎货查询菜单"
